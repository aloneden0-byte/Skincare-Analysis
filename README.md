...
## סוכן העשרת רכיבים (רץ אוטומטית ברקע)

`.github/workflows/enrich-ingredients.yml` רץ פעם ביום (וגם ניתן להפעלה ידנית מ-Actions → Enrich Ingredient Database → Run workflow): הוא מוצא רכיבים שסומנו "טרם דורג" (נוצרו אוטומטית בזמן סריקה של רכיב לא מוכר), חוקר כל אחד מהם מול Gemini API, וכותב חזרה דירוג קומדוגניות/גירוי/תועלת, קטגוריה, התאמת סוגי עור ותיאור קצר — כך שהמאגר משתפר ברקע בלי התערבות ידנית.

**הקמה (חד-פעמית, גם מהנייד):**
1. Google AI Studio → [aistudio.google.com/apikey](https://aistudio.google.com/apikey) → **Create API key** (חינמי, לא דורש כרטיס אשראי לרמת החינמית).
2. בריפו: **Settings → Secrets and variables → Actions** → הוסיפו:
   - `GEMINI_API_KEY` — המפתח מלמעלה
   - `SUPABASE_SERVICE_ROLE_KEY` — מ-Supabase Settings → API → **service_role** key (שונה מה-`anon`/`publishable` שכבר הוספתם — זה מפתח בעל הרשאות מלאות, אל תחשפו אותו בשום מקום אחר)

כל הרצה מטפלת בעד 15 רכיבים כדי להישאר בנוחות בתוך המכסה החינמית של Gemini — אם יש צבירה גדולה, היא תתעדכן על פני כמה ריצות, או אפשר להריץ ידנית כמה פעמים ברצף מ-Actions.
...
2. **ניתוח** — כל רכיב ברשימה מותאם (התאמה מדויקת ואז מטושטשת) מול מאגר הרכיבים ב-Supabase. רכיב לא מוכר נוסף כרשומה נייטרלית "טרם דורג" כדי לא לחסום את התהליך — וסוכן ברקע (ראו למטה) חוקר ומדרג אותו בהמשך.
...
scripts/
  enrich-ingredients.mjs   # סוכן ההעשרה - רץ מ-.github/workflows/enrich-ingredients.yml
src/
